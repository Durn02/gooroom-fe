...
export const reloadDataset = (
  ...
  const currentNodeMap: { [key: string]: Node } = currentNodes.reduce((map, node) => {
    if (node.id) {
      map[node.id] = node;
    }
    return map;
  ...
  const newNodeMap: { [key: string]: Node } = newNodes.reduce((map, node) => {
    if (node.id) {
      map[node.id] = node;
    }
    return map;
  ...
  const currentEdgeMap: { [key: string]: Edge } = currentEdges.reduce((map, edge) => {
    const edgeKey = `${edge.from}-${edge.to}`;
    map[edgeKey] = edge;
    return map;
  }, {} as { [key: string]: Edge });

  const newEdgeMap: { [key: string]: Edge } = newEdges.reduce((map, edge) => {
    const edgeKey = `${edge.from}-${edge.to}`;
    map[edgeKey] = edge;
    return map;
  }, {} as { [key: string]: Edge });
