...
async def send_verification_code(
    response: Response,
    driver=Depends(get_session),
    send_verification_code_request: SendVerificationCodeRequest = Body(...),
):
    logger.info("send verify code")

    try:
        # Verification count 조회
        query = """
        MATCH (p:PrivateData {email: $email})
        RETURN p.verification_count AS verification_count
        """
        session = driver.session()
        result = session.run(query, email=send_verification_code_request.email)
        ...
        # 새로운 verification count 및 정보 생성
        ...
        # Verification 정보 업데이트
        update_query = """
        MATCH (p:PrivateData {email: $email})
        SET p.verification_count = $new_verification_count, 
        ...
        result = session.run(
            ...
            new_verification_count=new_verification_count,
...
async def signup(
    ...
    try:
        ...
        query = """
        MERGE (p:PrivateData {email: $email})
        ON CREATE SET p.password = $password, 
                      ...
                      p.grant = 'not-verified'
        
        MERGE (u:User {username: $username})
        ON CREATE SET u.nickname = $nickname, 
                      u.concern = $concern, 
                      u.my_memo = ''
...
async def signin(
    ...
    MATCH (p:PrivateData {email: $email})
    MATCH (p)-[:is_info]->(u)
    RETURN p.password AS password, u.grant AS grant, ID(u) AS id
    """
